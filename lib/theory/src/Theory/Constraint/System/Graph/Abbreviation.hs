...
import           Term.LTerm
import           Theory                   (Fact(..), LNFact, Rule(..))
...
data AbbreviationOptions = AbbreviationOptions
  { abbrevsSoftLimit   :: Int    -- ^ The soft limit of how many abbreviations to generate if the term weight is not greater or equal than 'alwaysAbbrevWeight'.
  , alwaysAbbrevWeight :: Weight -- ^ Terms that have weight greater or equal to this always generate an abbreviation even if the number of abbreviations is greater than 'abbrevsSoftLimit'.
  , firstIndex         :: Int    -- ^ The first index to use when generating abbreviations.
  , prefixLength       :: Int    -- ^ The length of an abbreviation prefixes.
  }
  deriving( Eq, Ord )
...
defaultAbbreviationOptions = AbbreviationOptions
  { abbrevsSoftLimit = 10
  , alwaysAbbrevWeight = 30 -- a.d. TODO what is a good default value for this?
  , firstIndex = 1
  , prefixLength = 2
  }
...
getTermPrefix options t = map toUpper $ take options.prefixLength $ filter isAlpha go
  where
...
abbreviateTerm options allNames prefixMap t =
  ...
  where
    ...
    getIndexCandidate pref = fromMaybe options.firstIndex $ M.lookup pref prefixMap
...
computeAbbreviations repr options = makeRecursive $ M.toList $ go allTermOccs M.empty M.empty
  where
      ...
      in
        case candidateM of
          ...
          Just (candidate, weight) ->
            ...
            if weight < options.alwaysAbbrevWeight && length abbrevs >= options.abbrevsSoftLimit
            then abbrevs
            else
    ...
    allTermOccs =
      let terms = concatMap getNodeTerms repr.nodes ++ concatMap (\c -> concatMap getNodeTerms c.nodes) repr.clusters in
    ...
    getNodeTerms (Node _ (SystemNode ru)) =
      concatMap getFactTerms ru.prems
      ++ concatMap getFactTerms ru.acts
      ++ concatMap getFactTerms ru.concs
    ...
    getFactTerms fact = filter (not . isPair) $ concatMap getSubTerms fact.factTerms
