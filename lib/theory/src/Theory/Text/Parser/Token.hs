...
data ParserState = PState
       ...
       , userDefinedFunNames  :: S.Set String -- User defined function names, used to check for conflicts with reservedBuiltinNames
       }
       deriving( Eq, Ord, Show )

-- | A monoid instance to combine parser signatures.
instance Semigroup ParserState where
 PState sig1 flags1 rbn1 udfn1 <> PState sig2 flags2 rbn2 udfn2 =
   PState (sig1 <> sig2) (flags1 `S.union` flags2) (rbn1 ++ rbn2) (udfn1 `S.union` udfn2)

instance Monoid ParserState where
  mempty = PState {sig=mempty, flags = S.empty, reservedBuiltinNames = [], userDefinedFunNames = S.empty}
