...
data ProofStateRetention = RetainProofStates | ReleaseProofStates
    deriving (Eq, Show)
...
proveSystemDFS :: ProofStateRetention -> Heuristic ProofContext -> [Tactic ProofContext] -> ProofContext -> Int -> System -> Proof (Maybe System)
proveSystemDFS retention heuristic tactics ctxt initialDepth initialSystem =
  ...
  where
    ...
    -- Just does not force the state. Child states replay the recorded methods
    -- only when their contents are demanded.
    restoreStates se (LNode (ProofStep method _) cases) =
        LNode (ProofStep method (Just se)) $
