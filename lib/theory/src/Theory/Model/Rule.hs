...
import Theory.Model.Formula qualified as F
import Theory.Sapic
  ...
  if i0 == i1
    then
      if ps0 == ps1
        then
          if cs0 == cs1
            then compare as0 as1
            else compare cs0 cs1
        else compare ps0 ps1
    else compare i0 i1
...
data RuleAttribute
  = RuleColor (RGB Rational) -- Color for display
...
getSubstitutionsFixingNewVars
  (Rule (ProtoInfo (ProtoRuleACInstInfo _ _ _)) _ _ _ instancesO)
...
equalUpToTerms ruAC@(Rule _ ps cs as _) ruE@(Rule _ ps' cs' as' _) =
  ruleName ruE == ruleName ruAC
    ...
    && length cs == length cs'
    && length as == length as'
    && foldl sameFacts True (zip ps ps')
    && foldl sameFacts True (zip cs cs')
...
addDiffLabel (Rule info prems concs acts nvs) name =
  Rule
    ...
    ( acts
        ++ [ Fact
               { factTag = ProtoFact Linear name 0,
...
equalRuleUpToRenaming r1@(Rule rn1 pr1 co1 ac1 nvs1) r2@(Rule rn2 pr2 co2 ac2 nvs2) = reader $ \hnd ->
  ...
  where
    ...
    matchFacts (Just l) (Fact f1 _ t1, Fact f2 _ t2)
      | f1 == f2 = Just ((zipWith Equal t1 t2) ++ l)
      | otherwise = Nothing
...
equalRuleUpToDiffAnnotationSym ru1 ru2 =
  equalRuleUpToDiffAnnotation ru1 ru2
  ...
  where
    trimSapicName name =
      case splitString name of
        ...
        Nothing -> name
    splitString str =
      let parts = reverse $ splitOn "_" str
...
prettyNamedRule ::
  (HighlightDocument d, HasRuleName (Rule i), HasRuleAttributes (Rule i)) =>
  -- | Prefix.
  d ->
  -- | Rule info pretty printing.
  (i -> d) ->
  Rule i ->
  d
prettyNamedRule prefix ppInfo ru =
  prefix <-> prettyRuleName ru
    <> ppAttributes
    <> colon
      $-$ nest
        2
