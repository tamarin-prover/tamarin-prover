...
data RuleAttributes = RuleAttributes {
    ruleColor :: Maybe (RGB Rational), -- Color for display
    ruleProcess :: Maybe (PlainProcess), -- Process: for display, but also to recognise
    ...
    ignoreDerivChecks :: Bool,
    isSAPiCRule :: Bool, -- tags the rule as a SAPiC rule
    role :: Maybe String -- name of the rule
}
       deriving( Eq, Ord, Show, Data, Generic)
instance NFData RuleAttributes
instance Binary RuleAttributes
instance Semigroup RuleAttributes where
    (<>) r1 r2 = -- combine r1 with r2, giving precedence to the latter
    ...
    -- `x <> (y <> z) = (x <> y) <> z `
        ...
        where
            -- is associative
            preferRight _ (Just b) = Just b
            preferRight a Nothing = a

instance Monoid RuleAttributes where
    mempty = -- define "default attributes" when rule is given without attributes
    -- Also: needs to have identity in both directions
    -- x <> mempty = x
    -- mempty <> x = x
        RuleAttributes {
            ruleColor = Nothing, -- yup
            ruleProcess = Nothing, -- yup
            ignoreDerivChecks = False, -- yup
            isSAPiCRule = False, -- yup
            role = Nothing -- yup
        }
...
-- Instances
------------
instance Apply s RuleAttributes where
    apply _ = id

instance HasFrees RuleAttributes where
