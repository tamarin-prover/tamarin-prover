...
headerOfFunSym  (((f,(k,Public)),inTypes,Just outType):remainder)  (f2,(k2,Public)) =
  if f2==f && k2 == k then
    Fun ("fun " ++ BC.unpack f ++ "(" ++ (make_argtypes inTypes) ++ "):" ++ outType ++ ".")
  else headerOfFunSym remainder (f2,(k2,Public))
headerOfFunSym  (((f,(k,Private)),inTypes,Just outType):remainder)  (f2,(k2,Private)) =
  if f2==f && k2 == k then
    Fun ("fun " ++ BC.unpack f ++ "(" ++ (make_argtypes inTypes) ++ "):" ++ outType ++ "[private].")
  else headerOfFunSym remainder (f2,(k2,Public))
headerOfFunSym _ _ = Fun "Error: unsupported function declaration"
