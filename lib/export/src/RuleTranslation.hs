...
makeHeadersFromProtoRule ru thy = (freeHeaders, tables, events)
  where
    ...
    freeHeaders = makeFreeHeaders (facts rPrems) (notDiffRuleActs ru) (facts rConcs) thy
...
searchTermForBitstrings = foldMap (\ l -> if (head $ show l, last $ show l) == ('\'', '\'')
...
checkTypes ru thy = if null incorrectFunctionUsages then ru else throw $ UnsupportedTypes incorrectFunctionUsages
  where
    facts proj = L.get proj ru
    allFacts = facts rPrems ++ notDiffRuleActs ru ++ facts rConcs
...
incorrectTermTypes thy t = case viewTerm t of
    ...
    FApp (ACfct (f, _)) ts -> checkFun (BC.unpack f) ++ foldMap (incorrectTermTypes thy) ts
...
translateProtoRule ru de =
    ...
    where
      facts proj = L.get proj ru
      (factsDoc,destructors) = translateRule (facts rPrems) (notDiffRuleActs ru) (facts rConcs) de
      ruleDoc = text "let" <-> (text . showRuleName . L.get preName $ L.get rInfo ru) <-> text "=" $-$ nest 8 factsDoc
...
sanitizeSymbol pre s = if (s `List.elem` reservedWords) || Data.Char.isDigit (head s)
                         then pre : s
                         else s

reservedWords :: [String]
reservedWords = ["among", "axiom", "channel", "choice", "clauses", "const", "def", "diff",
...
printTerm sanitizeAtoms vars checkEq t = case viewTerm t of
    ...
    FApp (ACfct (f, _)) ts                          -> ppFunSym f ++ printTermsList ts
...
translatePatternTerm vars helperVars t = case viewTerm t of
    Lit l | S.member (show l) vars || head (show l) == '\'' -> (text "=" <> (text . showAtom True $ show l), helperVars)
    Lit l                                                   -> (text . showAtom True $ show l, helperVars)
