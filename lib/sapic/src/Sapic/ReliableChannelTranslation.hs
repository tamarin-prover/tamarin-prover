...
reliableChannelInit :: Monad m => LProcess ann -> ([AnnotatedRule ann], Set LVar) -> m ([AnnotatedRule ann], Set LVar)
...
reliableChannelTransAct tAct ac an p tx
            ...
            | (ChOut (Just _) _) <- ac = throwM  WFReliable
            | (ChIn (Just _) _ _) <- ac = throwM WFReliable
            | (ChOut Nothing _) <- ac = throwM WFReliable
            | (ChIn Nothing _ _) <- ac = throwM WFReliable
...
reliableChannelRestr :: (MonadThrow m, MonadCatch m, Show ann) => LProcess ann -> [SyntacticRestriction] -> m [SyntacticRestriction]
