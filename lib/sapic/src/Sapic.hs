...
translate th =
  case theoryProcesses th of
    ...
    [p] -> do
      ...
      eProtoRule <-  checkOps (.transProgress)
                        (pathCompression ops.compressEvents) eProtoRules'
...
gen (trans_null, trans_action, trans_comb) anP p tildex = do
  ...
  case proc' of
    ...
    where
      map_prems f = map (over #prems (map f))
...
isPosNegFormula fm = case fm of
    TF  _            -> (True, True)
    Ato (Action _ f) -> isActualKFact f.factTag
    Ato _            -> (True, True)
...
lemmaNeedsInEvRes lem = case (lem.traceQuantifier, isPosNegFormula lem.formula) of
