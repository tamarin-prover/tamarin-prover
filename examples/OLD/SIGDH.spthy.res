...
section{* Protocol Rules *}
 ...
 rule (modulo E) sigdh_I_1:
   ...
   -->
   [ sigdh_I_1( $I, $R, ~ni ),
     Send( <$I, sign{<'1', 'g'^~ni, $R>}sk(lts($I))> ) ]
   /* has exactly the trivial AC variant */

 rule (modulo E) sigdh_I_2:
   [ sigdh_I_1( $I, $R, ~ni ),
     Knows( <$R, sign{<'2', Gr, $I>}sk(lts($R))> ) ]
   -->
   [ sigdh_I_2( $I, $R, ~ni, Gr ) ]
   /* has exactly the trivial AC variant */

 rule (modulo E) sigdh_R_1:
   [ Knows( <I, sign{<'1', Gi, $R>}sk(lts(I))> ) ]
   -->
   [ sigdh_R_1( I, $R, Gi ) ]
   /* has exactly the trivial AC variant */

 rule (modulo E) sigdh_R_2:
   [ sigdh_R_1( I, $R, Gi ), Fresh( ~nr ) ]
   -->
   [ sigdh_R_2( I, $R, Gi, ~nr ),
     Send( <$R, sign{<'2', 'g'^~nr, I>}sk(lts($R))> ) ]
   /* has exactly the trivial AC variant */
...
lemma (modulo E) I_secrecy_ephkeys:
  "((#v :> sigdh_I_2( $I, $R, ~ni, Gr )) & (#u :> Knows( ~ni ))) ==>
...
lemma (modulo E) R_secrecy_ephkeys:
  "((#v :> sigdh_R_2( I, $R, Gi, ~nr )) & (#u :> Knows( ~nr ))) ==>
