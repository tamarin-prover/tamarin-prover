...
let fact2string (f:fact) = match f with 
  ...
  |    State(p,t)->  "State_"^(pos2string p)^"("^(flatten_varlist (VarSet.elements t))^")" 
  |   PState(p,t)->  "!State_"^(pos2string p)^"("^(flatten_varlist (VarSet.elements t))^")" 
  |    Semistate(p,t)->  "Semistate_"^(pos2string p)^"("^(flatten_varlist (VarSet.elements t))^")" 
  |    PSemistate(p,t)->  "!Semistate_"^(pos2string p)^"("^(flatten_varlist (VarSet.elements t))^")" 
