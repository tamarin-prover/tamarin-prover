...
data FactTag = ProtoFact Multiplicity String Int
             ...
             | SendFact   -- ^ Sent by the protocol
             | KnowsFact  -- ^ Officially known by the intruder/network.
...
-- | A send fact denotes a message sent by the protocol to the intruder.
sendFact :: t -> Fact t
sendFact = Fact SendFact . return

-- | A fresh fact denotes a fresh unguessable name.
freshFact :: t -> Fact t
freshFact = Fact FreshFact . return

-- | A knows fact denotes the statement that the intruder knows a message and
-- hence the protocol can receive it.
knowsFact :: t -> Fact t
knowsFact = Fact KnowsFact . return
