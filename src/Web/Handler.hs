...
import Theory
  ( ClosedDiffTheory, ClosedTheory, Side
  , removeLemma, removeLemmaDiff, removeDiffLemma
...
import TheoryObject (Theory(..), DiffTheory(..))
...
-- | Store a theory, pure index.
putTheory
  :: (GenericTheoryInfo theory -> EitherTheoryInfo)
  -> Maybe (GenericTheoryInfo theory)  -- ^ Index of parent theory
  -> theory                            -- ^ The new closed theory
  -> Maybe TheoryOrigin                -- ^ Origin of this theory
  -> String
  -> Handler TheoryIdx
putTheory mkEti parent thy origin rep = do
  yesod <- getYesod
  liftIO $ modifyMVar yesod.theoryVar $ \theories -> do
    ...
    let idx | M.null theories = 1
        ...
        newOrigin    = fromMaybe Interactive (parentOrigin <|> origin)
        newThy       = mkEti $
          TheoryInfo idx thy time parentIdx False newOrigin
...
modifyTheory ti f fpath errResponse = do
  ...
  case res of
    ...
    Right (Just thy) -> do
      newThyIdx <- putTheory Trace (Just ti) thy Nothing ti.errorsHtml
...
modifyDiffTheory ti f fpath errResponse = do
  ...
  case res of
    ...
    Right Nothing    -> pure (responseToJson errResponse)
    Right (Just thy) -> do
      newThyIdx <- putTheory Diff (Just ti) thy Nothing ti.errorsHtml
  ...
  case result of
    ...
    Just fileinfo -> do
      ...
      if null content then
        setMessage "No theory file given."
      else do
        yesod <- getYesod
        thyWithRep <- liftIO $ runExceptT $ do
          openThy <- yesod.loadThy (T.unpack $ T.decodeUtf8 $ BS.concat content) (T.unpack $ fileName fileinfo)
          ...
          Right (report, thy) -> do
            let wfErrors = if null report
                  then ""
                       ...
                       <> "\n</div>"
            let origin = Just $ Upload $ T.unpack $ fileName fileinfo
            void $ either (putTheory Trace Nothing)
                          (putTheory Diff Nothing) thy origin wfErrors
...
getTheoryMessageDeductionR idx =
  withBothTheory idx
    (pure . RepPlain . toContent . prettyRender)
    (pure . RepPlain . toContent . prettyRenderDiff)
  where
...
getTheoryMessageDeductionDiffR idx =
  withBothTheory idx
    (pure . RepPlain . toContent . prettyRender)
    (pure . RepPlain . toContent . prettyRenderDiff)
  where
  ...
  where
    goDiff ti =
      modifyDiffTheory ti
        ...
        (JsonAlert $ "Sorry, but " <> name <> " failed!")
      where
        namesDiff thy = map (._lDiffName) $ getDiffLemmas thy
        ...
        proveAllDiff thy = pure $ proveDiff thy >>= (\thb -> foldM (\tha (s, lemma) -> applyProverAtPathDiff tha s lemma [] autoProver) thb $ names thb)
...
getAutoProverR idx extractor bound =
  ...
  where
    ...
    fullName   = proverName <> " (" <> T.intercalate ", " qualifiers <> ")"
...
getTheoryGraphR idx path = withTheory idx ( \ti -> do
  ...
  img <- liftIO $ traceExceptions "getTheoryGraphR" $
    imgThyPath
      ...
      abbreviate
      ti.theory path
  sendFile (fromString . imageFormatMIME $ yesod.imageFormat) img)
  where
