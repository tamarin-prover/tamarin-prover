...
withWebUI
  ...
  -> IO b
withWebUI readyMsg cacheDir thDir loadState autosave thyOpts loadThy closeThy debug
          graphCmd imageFormat defaultAutoProver f = do
  thy <- getTheos
  ...
  where
    ...
    getTheos = do
      ...
      if loadState && existsAutosave then do
       ...
       else loadTheories thyOpts readyMsg thDir loadThy closeThy defaultAutoProver
