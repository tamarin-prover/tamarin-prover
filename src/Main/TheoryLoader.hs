...
data TheoryLoadOptions = TheoryLoadOptions
  ...
  , autoSources       :: Bool
  , verboseMode       :: Bool
  , outputModule      :: ModuleType -- Note: This flag is only used for batch mode.
  ...
  where
    ...
    outputModule = case findArg "outModule" as of
      Just str -> case find ((str ==) . show) [minBound..] of
        Just m  -> pure m
        Nothing -> throwError $ ArgumentError "output mode not supported."
...
lemmaSelectorByModule thyOpt lem = case lemmaModules of
  ...
  _  -> thyOpt.outputModule `elem` lemmaModules
  where
...
processOpenTheory thyOpts = case modType of
  ...
  ModuleMsr                 -> Sapic.typeTheory >=> Sapic.translate >=> Acc.translate >=> (pure . filterLemma lemmas)
...
checkTranslatedTheory thyOpts sign thy = do
  ...
  variableReport <- if derivChecks == 0
    then pure $ Just []
    else do
      traceM ("[Theory " ++ theoryName thy ++ "] Derivation checks started")
      derivCheckSignature <- liftIO $ toSignatureWithMaude thyOpts.maudePath $ maudePublicSig (toSignaturePure sign)
...
withVersionAndReport version thyOpts report thy = do
  let
    reportThy = bimap (addComment     (reportToDoc report))
                      (addDiffComment (reportToDoc report))
                      thy

    versionThy = bimap (addComment (Pretty.text version))
  ...
  pure versionThy
  where
    quitOnWarning = thyOpts.quitOnWarning
...
closeTranslatedTheory thyOpts sign srcThy = do
  ...
  where
    ...
    selector = lemmaSelectorByModule thyOpts &&& lemmaSelector thyOpts
...
closeTheory version loadedThyOpts sign srcThy = do
  ...
  where
    ...
    configAutoSources opts = opts { autoSources = argExists "auto-sources" srcThyConfigBlockArgs || loadedAutoSources }
...
prettyOpenTheoryByModule thyOpts = case modType of
  ModuleSpthy               -> pure . prettyOpenTheory
  ModuleSpthyTyped          -> pure . prettyOpenTheory
  ModuleMsr                 -> pure . prettyOpenTranslatedTheory . removeTranslationItems
...
addParamsOptions opt = addVerboseOptions . addSatArg . addChainsArg . addLemmaToProve
  where
    ...
    chain = opt.openChain
    addChainsArg (Left thy) = Left thy { _thyOptions = thy._thyOptions { _openChainsLimit = chain }}
    addChainsArg (Right diffThy) = Right diffThy { _diffThyOptions = diffThy._diffThyOptions { _openChainsLimit = chain }}
    -- Add Saturation Limit parameters in the Options
    sat = opt.saturation
    addSatArg (Left thy) = Left thy { _thyOptions = thy._thyOptions { _saturationLimit = sat }}
    addSatArg (Right diffThy) = Right diffThy { _diffThyOptions = diffThy._diffThyOptions { _saturationLimit = sat }}
    -- Add lemmas to Prove in the Options
    lem = opt.lemmaNames
    addLemmaToProve (Left thy) = Left thy { _thyOptions = thy._thyOptions { _lemmasToProve = lem }}
    addLemmaToProve (Right diffThy) = Right diffThy { _diffThyOptions = diffThy._diffThyOptions { _lemmasToProve = lem }}
    -- Add Verbose parameter in the Options
    verb = opt.verboseMode
    addVerboseOptions (Left thy) = Left thy { _thyOptions = thy._thyOptions { _verboseOption = verb }}
    addVerboseOptions (Right diffThy) = Right diffThy { _diffThyOptions = diffThy._diffThyOptions { _verboseOption = verb }}
