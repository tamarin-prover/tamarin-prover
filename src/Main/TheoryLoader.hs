...
loadTheory thyOpts input inFile = do
  ...
  where
    thyParser | thyOpts.diffMode = Right $ diffTheory $ Just inFile
...
checkTranslatedTheory thyOpts sign thy = do
  ...
  where
    ...
    makepublic = Set.map (\(name, (int, _, construct)) -> (name, (int, Public, construct)))
    makepublicsym = Set.map $ \case
...
withVersionAndReport version thyOpts report thy = do
  ...
  when (thyOpts.quitOnWarning && not (null report)) $
    throwError (WarningError report)
...
closeTheory version loadedThyOpts sign srcThy = do
  ...
  where
    ...
    thyHeurDefOracle opts =
      opts { heuristic = (\(Heuristic grl) -> Heuristic $ defaultOracleNames srcThyInFileName grl) <$> loadedThyOpts.heuristic }
    ...
    configStopOnTrace opts =
      if isNothing loadedThyOpts.stopOnTrace
        then opts { stopOnTrace = either (\(ArgumentError e) -> error e) id $ stopOnTrace srcThyConfigBlockArgs }
        else opts

    configAutoSources opts =
      opts { autoSources = argExists "auto-sources" srcThyConfigBlockArgs || loadedThyOpts.autoSources }
